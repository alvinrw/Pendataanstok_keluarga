...
print("=" * 60)
print("IMPORTING PENJUALAN DATA FOR KLOTER 1 (ID=7)")
print("=" * 60)

# Get Kloter 1 ID from database
cursor.execute("SELECT id, nama_kloter FROM kloters WHERE nama_kloter LIKE '%Kloter 1%' ORDER BY id LIMIT 1")
kloter_row = cursor.fetchone()

if not kloter_row:
    print("[ERROR] Kloter 1 not found in database!")
    conn.close()
    exit()
...
# Read CSV and import penjualan for kloter_id = 7 (from old database)
with open(CSV_FILE, 'r', encoding='utf-8') as f:
    ...
    for row in reader:
        # Only import if kloter_id = 7 (Kloter 1 in old database)
        if row['kloter_id'] != '7':
            continue
...
# Recalculate summary
print("\n[RECALCULATING] Summary for Kloter 1...")
